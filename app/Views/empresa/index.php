    ...
    <div class="w-full max-w-2xl">
        <div class="glass-card p-8 rounded-xl">
            <form id="formConfigEmpresa" class="space-y-4">
                <!-- Sección del Logo -->
                <div
                    class="flex flex-col items-center mb-6 p-4 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Logo Distintivo del
                        Taller</p>
                    <div class="relative group">
                        <img id="logo-preview"
                            src="<?php echo !empty($data['config']->logo) ? URLROOT . '/' . $data['config']->logo : URL_IMG . 'default.png'; ?>"
                            alt="Vista previa" class="w-32 h-32 object-contain rounded-lg shadow-md bg-white p-2">
                        <label for="logoInput"
                            class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer rounded-lg">
                    ...
                    <p class="text-[9px] text-slate-400 mt-2 italic text-center">Formatos sugeridos: PNG o JPG (Fondo
                        transparente preferiblemente)</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Días de Garantía para
                            Devoluciones</label>
                        <input type="number" name="dias_garantia_devolucion" id="config-dias-garantia" min="1"
                            class="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-neon-green outline-none"
                            value="<?php echo s($data['config']->dias_garantia_devolucion ?? 5); ?>" required>
                        <p class="text-[10px] text-slate-400 mt-1">Días máximos para aceptar devoluciones de repuestos
                            (valor global).</p>
                    </div>
                </div>
                <div>
