<?php
    ...
    $titulo_pestaña = ($venta->id_formateado ?: "Factura N " . $venta->id) . ($es_os ? " - OS #".$venta->orden_id : "");
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    ...
    ?>
    <div class="obs-box">
        <div class="label-min">Observaciones y Detalles Técnicos:</div>
        <div style="font-size: 9px; margin-top: 5px; line-height: 1.4;">
            <?php 
                if($es_os && !empty($obs_entrada)) {
                    echo "<strong>DIAGNÓSTICO DE INGRESO:</strong> " . htmlspecialchars($obs_entrada) . "<br>";
                }
                
                if(!empty($obs_salida) && !$identicas) {
                    if($es_os && !empty($obs_entrada)) echo "<div style='margin-top:4px;'></div>";
                    echo "<strong>NOTAS DE SALIDA / RECOMENDACIONES:</strong> " . htmlspecialchars($obs_salida);
                }
            ?>
        </div>
    </div>
