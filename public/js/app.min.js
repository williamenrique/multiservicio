...
async function initProfitabilityCard() {
    ...
    try {
        ...
        if (result.profitability) {
            ...
            const utilidad = parseFloat(p.utilidad_bruta || 0);
            const margen = totalVentas > 0 ? ((utilidad / totalVentas) * 100).toFixed(1) : 0;

            container.innerHTML = `
                <div class="glass-card rounded-2xl border border-emerald-100 shadow-sm overflow-hidden animate-in fade-in slide-in-from-right-2 duration-500">
                    <div class="bg-emerald-50/50 px-6 py-4 border-b border-emerald-100 flex justify-between items-center">
                        ...
                        <div class="text-right">
                            <p class="text-2xl font-black text-emerald-600 leading-none">${AppUtils.formatCurrency(utilidad)}</p>
                            <p class="text-[11px] font-bold text-emerald-400 uppercase tracking-widest mt-1">Ganancia Real</p>
                        </div>
                    </div>
                    <div class="p-6">
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-[10px] font-black text-slate-400 uppercase">Margen de Utilidad</span>
                            <span class="text-xs font-black text-emerald-600">${margen}%</span>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mt-6 pt-4 border-t border-slate-50">
                            <div>
                                <p class="text-[10px] font-black text-slate-400 uppercase">Total Vendido</p>
                                <p class="text-sm font-bold text-navy-blue">${AppUtils.formatCurrency(totalVentas)}</p>
                            </div>
                            <div>
                                <p class="text-[10px] font-black text-slate-400 uppercase">Costo Mercancía</p>
                                <p class="text-sm font-bold text-rose-500">${AppUtils.formatCurrency(p.total_costos)}</p>
                            </div>
                        </div>
                    </div>
                </div>
