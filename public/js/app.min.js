...
async function initWorkshopAlerts() {
    ...
    const containerHeader = document.getElementById('workshop-notifications-container');
    ...
    try {
        ...
        const tarde = alertas.filter(a => a.es_tarde);
        const proximas = alertas.filter(a => a.es_proximo);

        // 1. Renderizado en el Index de Taller (Versión Compacta - Píldoras)
        if (containerIndex) {
            if (tarde.length > 0 || proximas.length > 0) {
                containerIndex.classList.remove('hidden');
                containerIndex.innerHTML = `
                    <div class="col-span-full flex flex-wrap gap-2 mb-4">
                        ${tarde.map(a => `
                            <div class="flex-1 min-w-[180px] bg-rose-600 text-white px-3 py-1.5 rounded-lg shadow-sm alert-shake flex items-center justify-between border border-rose-400">
        ...
        if (containerHeader) {
            if (tarde.length > 0 || proximas.length > 0) {
                const total = tarde.length + proximas.length;
                containerHeader.innerHTML = `
                <div class="relative group">
                    <button class="p-2 bg-blue-500/10 text-blue-500 rounded-lg border border-blue-500/20 ${tarde.length > 0 ? 'alert-shake' : 'animate-pulse'}" title="Alertas de Entrega">
                        <i data-lucide="wrench" class="w-5 h-5"></i>
                        <span class="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-black px-1.5 rounded-full border-2 border-white">${total}</span>
                    </button>
                    <div class="hidden group-hover:block absolute top-full right-0 w-72 pt-2 z-50">
                        <div class="bg-black shadow-2xl rounded-xl p-3 border border-slate-700 text-left">
                            <p class="text-[10px] font-bold text-white uppercase mb-2 tracking-widest">Vehículos por Entregar</p>
                            <div class="max-h-60 overflow-y-auto space-y-2 custom-scrollbar">
                                ${tarde.map(a => `<div class="text-xs border-b border-slate-800 pb-2"><p class="text-rose-500 font-black text-[9px] uppercase">¡Atrasado!</p><p class="text-white font-bold uppercase">Placa: ${a.placa}</p></div>`).join('')}
                                ${proximas.map(a => `<div class="text-xs border-b border-slate-800 pb-2"><p class="text-amber-400 font-black text-[9px] uppercase">En ${Math.round(a.tiempo)} min</p><p class="text-white font-bold uppercase">Placa: ${a.placa}</p></div>`).join('')}
                            </div>
                            <a href="${URLROOT}/taller" class="block text-center text-[10px] font-bold text-blue-500 mt-2 uppercase hover:underline">Ver Panel Operativo</a>
                        </div>
                    </div>
                </div>`;
            } else {
