        ...
        if (container && window.currentLoggedInUser && (parseInt(window.currentLoggedInUser.roleId) === 1 || window.currentLoggedInUser.role.toUpperCase() === "ADMINISTRADOR")) {
            if (lowStock.length > 0) {
                ...
                <div class="relative group">
                    ...
                    <div class="hidden group-hover:block absolute top-full right-0 w-64 pt-2 z-50">
                        <div class="bg-white shadow-2xl rounded-xl p-3 border border-slate-100 text-left">
                            <p class="text-[10px] font-bold text-slate-400 uppercase mb-2 tracking-widest">Stock Crítico</p>
                            <div class="max-h-60 overflow-y-auto space-y-2">
                                ${lowStock.map(p => `<div class="text-xs border-b border-slate-50 pb-1"><p class="font-bold text-navy-blue uppercase leading-tight">${p.nombre}</p><p class="text-slate-400 text-[10px]">Stock: <span class="text-rose-500 font-black">${p.stock}</span> / Mín: ${p.stock_minimo}</p></div>`).join("")}
                            </div>
