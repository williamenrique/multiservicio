...
window.printInvoice = async (ventaId) => {
    ...
    else AppUtils.showAlert("Error", data.mensaje, "error");
};
...
const CajaUI = {
    ...
    checkStatus: async () => {
        try {
            const res = await fetch(`${URLROOT}/caja/estado`);
            const data = await res.json();
            CajaUI.status = data.abierta ? data.sesion : null;
            CajaUI.renderIndicator();
        } catch (e) { console.error("Error caja:", e); }
    },

    renderIndicator: () => {
        const container = document.getElementById('caja-status-container');
        if (!container) return;

        if (CajaUI.status) {
            container.innerHTML = `
                <button onclick="CajaUI.init()" class="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 text-emerald-500 rounded-lg border border-emerald-500/20 hover:bg-emerald-500/20 transition-all">
            ...
            container.innerHTML = `
                <button onclick="CajaUI.init()" class="flex items-center gap-2 px-3 py-1.5 bg-rose-500/10 text-rose-500 rounded-lg border border-rose-500/20 hover:bg-rose-500/20 transition-all">
                    <i data-lucide="lock" class="w-3 h-3"></i>
                    <span class="text-[10px] font-black uppercase tracking-wider">Caja Cerrada</span>
                </button>`;
        }
        window.lucide && lucide.createIcons();
    },
    ...
    init: async () => {
        if (!CajaUI.status) {
            // Verificar si el usuario actual es administrador (Rol 1)
            const user = window.currentLoggedInUser;
            ...
            if (!isAdmin) {
                return AppUtils.showAlert(
                    "Caja Cerrada",
                    "Debe solicitar a un administrador que realice la apertura de caja para poder facturar.",
                    "warning"
                );
            }
            ...
            const { value: monto } = await Swal.fire({
                title: 'Abrir Caja',
                text: 'Ingrese el monto inicial de efectivo en caja',
                input: 'number',
            ...
            if (monto !== undefined) {
                const res = await fetch(`${URLROOT}/caja/abrir`, {
                    method: 'POST',
            ...
            if (!isAdmin) {
                return AppUtils.showAlert(
                    "Acceso Denegado",
                    "Solo un administrador puede realizar el arqueo y cierre de la caja.",
                    "warning"
                );
            }

            // Obtener saldo esperado antes de mostrar modal
            const resTotal = await fetch(`${URLROOT}/caja/estado`);
            const info = await resTotal.json();

            // En un sistema real, el sistema calcularía esto en el backend, 
            // aquí simulamos el flujo de cierre pidiendo el arqueo físico.
            const { value: montoReal } = await Swal.fire({
                title: 'Arqueo de Caja',
                html: `
                    <div class="text-left space-y-3 p-2">
                        <div class="p-3 bg-slate-50 rounded-lg border border-slate-200">
                            <p class="text-[10px] font-black text-slate-400 uppercase">Monto Inicial</p>
                            <p class="text-lg font-bold text-navy-blue">${AppUtils.formatCurrency(info.sesion.monto_inicial)}</p>
                        </div>
                        <p class="text-[11px] text-slate-500 text-center italic">Cuente el efectivo total que tiene físicamente en el cajón.</p>
                    </div>
                `,
                input: 'number',
                inputLabel: 'Monto Total Físico',
                inputPlaceholder: '¿Cuánto dinero hay?',
                showCancelButton: true,
                confirmButtonText: 'REALIZAR CIERRE',
                confirmButtonColor: '#e11d48',
                inputValidator: (value) => {
                    if (!value) return 'Debe ingresar el monto contado físicamente';
                }
            });

            if (montoReal !== undefined) {
                AppUtils.showLoading('Procesando arqueo...');
                const res = await fetch(`${URLROOT}/caja/cerrar`, {
                    method: 'POST',
                ...
                if (result.success) {
                    await AppUtils.showAlert("Caja Cerrada", result.mensaje, "success");
                    CajaUI.checkStatus();
                } else {
                    AppUtils.showAlert("Error", result.mensaje, "error");
                }
